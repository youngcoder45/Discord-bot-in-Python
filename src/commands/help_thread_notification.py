    ...
    async def on_thread_create(self, thread):
        ...
        try:
            ...
            if hasattr(thread, "applied_tags") and thread.applied_tags:
                ...
                if tag_names:
                    embed.add_field(
                        name="Tags", value=", ".join(tag_names), inline=False
                    )
            ...
            await target_channel.send(
                content="@here There's a New Help Forum(<#1388169643234955354>), Do check if u can help",
                embed=embed,
            )
            ...
            logger.error(
                f"Error in help thread notification for thread {thread.id}: {e}",
                exc_info=True,
            )
